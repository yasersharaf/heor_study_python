...
******************************************************************************************************/
    ...
    /* Calculate Charlson index, reduce to 1 obs per patient */
    proc means data=work.comb_charlson nway noprint;
        var cc_grp_1-cc_grp_17 ;
        class &id;
        output out = work.comb_out max = cc_grp_1-cc_grp_17;
    run;
    ...
    data &src..&outdata;
        ...
        /* charlson index*/
        charlson_index = sum(of cc_grp_1-cc_grp_10, 0) * 1 
                        + sum(of cc_grp_11-cc_grp_14, 0) * 2
                        + sum(cc_grp_15, 0) * 3
                        + sum(of cc_grp_16-cc_grp_17, 0) * 6;
        label charlson_index = 'Charlson Comorbidity Index';

        keep &id charlson_index cc_grp_1-cc_grp_17;
     run;
%Mend COMB_Charlson;
